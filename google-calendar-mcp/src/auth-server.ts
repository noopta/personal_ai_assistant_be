...
async function runAuthServer() {
  ...
  try {
    ...
    logger.info('Starting auth serverv from auth-server.ts');
    authServer = new AuthServer(oauth2Client, userID);
    ...
    if (!success && !authServer.authCompletedSuccessfully) {
      ...
      process.stderr.write('Authentication failed. Could not start server or validate existing tokens. Check port availability (3003-3007) and try again.\n');
      process.exit(1);
